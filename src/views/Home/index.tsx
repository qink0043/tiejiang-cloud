...
import { Card, Button, Input, Upload, message, Space, Table, Dropdown } from 'antd';
import { SearchOutlined, UploadOutlined, FolderAddOutlined, DownloadOutlined, DeleteOutlined, MoreOutlined, FileTextOutlined, FolderOutlined, VideoCameraOutlined, AudioOutlined, FileUnknownOutlined } from '@ant-design/icons';
...
const getFileIcon = (type: string, extension?: string) => {
  ...
  switch (extension?.toLowerCase()) {
    ...
    case 'avi':
    case 'mov':
      return <VideoCameraOutlined style={{ fontSize: '18px', color: '#fa8c16' }} />;
    case 'mp3':
    case 'wav':
    case 'flac':
    ...
    case 'rar':
    case '7z':
      return <FileUnknownOutlined style={{ fontSize: '18px', color: '#13c2c2' }} />;
    default:
      return <FileUnknownOutlined style={{ fontSize: '18px', color: '#d9d9d9' }} />;
  }
};
  ...
  const columns: ColumnsType<FileItem> = [
    {
      ...
      render: (name: string, record: FileItem) => (
        <Space>
          ...
          <span className={record.type === 'folder' ? 'file-folder' : 'file-name'}>
            {name}
          </span>
        </Space>
      ),
    ...
    <div className="home-container">
      ...
      <div className="home-header">
        <Space>
          ...
          <Button type="default" icon={<FolderAddOutlined />} onClick={handleCreateFolder}>
            新建文件夹
          </Button>
        </Space>
      </div>
      ...
      <Card className="file-list-card">
        <Table
          ...
          rowClassName={(record) => record.type === 'folder' ? 'folder-row' : 'file-row'}
          onRow={(record) => ({
            onClick: () => {
